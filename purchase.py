...
3) show the last update of the product
4) write the invoice for customer with unique naming
'''
def purchase(List):
    ...
    while flag.upper()=="Y":  #check and go if flag is 'Y' or 'y'.
        ...
        if product_name==L[0][0].upper():
            print('It is Basic model with 200MB RAM Size with free Earphone with Box and 1 Year Warranty with it!')
        elif product_name==L[1][0].upper():
            print('It is Pentium(R) DualCore Laptop with 1GB of RAM and 200GB of HardDisk!')
        elif product_name==L[2][0].upper():
            print('It is of 1TB storage size')
        else:
    ...
    if (f_amount>=10000):
        ...
        print("You got your expected discount and amount is: ",dis)

    else:
        dis=0.0
        total=float(f_amount-dis)
        print("You did not got your expected discount\nBecause, your totel purchase did not meet the minimum criteria for 500Rs discount.")
    print("Your payable amount is: ", total)

    '''
        In the following operation:
    
        1) write a purchase product name and details in file (invoice).
        2) write a discounted amount and final payable amount in file (invoice).
    '''
    ...
    file.write("\n-------------------------------------------------------------")
    file.write("\n\t\t\t Your discounted amount is: "+str(dis))
    file.write("\n-------------------------------------------------------------")
    file.write("\n\t\t\t Your final amount is: "+str(total))
    file.write("\n-------------------------------------------------------------")
